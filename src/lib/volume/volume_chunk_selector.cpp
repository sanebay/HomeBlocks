...
VolumeChunkSelector::allocate_init_chunks_from_pdev(uint64_t init_chunks, uint64_t total_chunks) {
    ...
    if (result.size() != init_chunks) {
        // If we didnt get chunks we requested, fail the request
        // and release what we got back to pool.
        for (auto& chunk : result) {
            m_per_dev_chunks[chunk->get_pdev_id()].emplace(chunk->get_chunk_id(), chunk);
        }

        for (auto& [pdev, chunks] : m_per_dev_chunks) {
            LOGINFO("pdev: {} num_chunks: {}", pdev, chunks.size());
        }
