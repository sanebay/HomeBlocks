...
void VolumeChunkSelector::add_chunk(homestore::cshared< Chunk >& chunk) {
    ...
    LOGDEBUG("Adding chunk id to selector {} {}", m_module_name, chunk_id);
}
...
homestore::cshared< Chunk > VolumeChunkSelector::select_chunk(homestore::blk_count_t nblks,
    ...
    do {
        ...
        // If the ratio of available_blks to total_blks is less than half and there is room for more chunks then resize.
        if (((float)available_blks / total_blks) < 0.5 && (volc->num_active_chunks.load() < volc->max_num_chunks)) {
        ...
        LOGINFO("Not getting any chunks");
...
void VolumeChunkSelector::resize_volume_num_chunks(shared< VolumeChunksInfo > volc) {
...
#endif
    if (check_usage && ((float)available_blks / total_blks) > 0.5) {
        // Check again if another thread already did the resize.
        LOGI("Another thread already did the resize op");
        return;
    }
    ...
    iomanager.run_on_forget(iomgr::reactor_regex::random_worker, [volc, this]() mutable {
        std::string str;
        auto num_chunks_to_alloc =
            std::min(num_chunks_per_resize, (volc->max_num_chunks - volc->num_active_chunks.load()));
        auto chunks = allocate_chunks_from_pdevs(num_chunks_to_alloc);
